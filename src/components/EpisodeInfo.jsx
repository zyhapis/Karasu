...
const EpisodeInfo = () => {
    ...
    useEffect(() => {
       setOgTitle(`Karasu - ${animeInfo.title?.romaji} Episode ${currentEpisode?.number}`);
    ...
    const handleDownload = () => {
        ...
        const episodeName = `${animeInfo.title?.romaji}_episode_${currentEpisode?.number}[Karasu]`;

        HlsDownloader(streamLink?.url, episodeName);
    };

  return (
        <main className='flex flex-col gap-4 dark:text-white pb-8 lg:pt-40 lg:px-16 lg:grid lg:grid-cols-3 lg:gap-x-8 xl:px-28'>
            ...
            <Helmet prioritizeSeoTags>
                ...
                <section className='px-5 py-4 flex flex-col gap-4 sm:px-7 md:px-10 lg:px-0 lg:col-span-2'>
                    ...
                    <select onChange={(e) => setStreamQuality(e.target.value)} value={streamQuality} name="quality" id="quality" className='mx-auto px-2 dark:text-[#999] bg-white dark:bg-transparent outline outline-1 outline-gray-400 rounded-md sm:text-lg'>
