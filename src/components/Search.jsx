...
const Search = () => {
  const {ogTitle, setOgTitle, ogDesc, ogImg} = useContext(DataContext);
    ...
    useEffect(() => {
      setOgTitle(`Karasu - Search Results for "${query}"`);
    }, [])
    ...
    <main className='p-5 flex flex-col gap-3 sm:px-7 md:px-10 lg:pt-24 lg:px-16 xl:px-28'>
      ...
      <Helmet prioritizeSeoTags>
        ...
        <section className='font-montserrat flex justify-evenly items-center gap-6 border-b dark:border-b-gray-700 dark:text-white sm:text-lg lg:text-xl'>
            <button onClick={(e) => setActiveTab(e.target.textContent.toLowerCase())} type='button' className='px-4 pb-1 hover:text-[#1a1a1a] dark:hover:text-white' style={activeTab === 'anime' ? {borderBottom: '2px solid #286dd4', fontWeight: 700 } : {color: "#9ca3af"} }>Anime</button>

            <button onClick={(e) => setActiveTab(e.target.textContent.toLowerCase())} type='button' className='px-4 pb-1 hover:text-[#1a1a1a] dark:hover:text-white' style={activeTab === 'manga' ? {borderBottom: '2px solid #286dd4', fontWeight: 700} : {color: "#9ca3af"} }>Manga</button>
        </section>
